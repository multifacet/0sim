                            ...
                            const int tier)
{
    ...
    BUG_ON(page->private != 0xDEADBEEF);
    page->private = tier;
                                 ...
                                 const struct kernel_param * kp)
{
    ...
    for (i = 0; i < NUM_TIERS; i++) {
        ...
        while (n) {
            ...
            BUG_ON(page->private & RECLAIM_FLAG);
                                              ...
                                              struct page **current_page)
{
    ...
    while (*current_tier < NUM_TIERS) {
        if (list_empty(&pool->used_pages[*current_tier])) {
            ...
            if (chosen->private & RECLAIM_FLAG) {
                continue;
            }
...
int ztier_reclaim_page(struct ztier_pool *pool, unsigned int retries)
{
    ...
    while (retries-- > 0) {
        ...
        reclaim_tier = page->private & TIER_MASK;
        ...
        INIT_LIST_HEAD(&page->lru);
        list_add(&page->lru, &pool->used_pages[reclaim_tier]);
        page->private &= ~RECLAIM_FLAG;
