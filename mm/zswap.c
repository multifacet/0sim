...
#define RADIX_BITMAP_VAL_MASK(val) (GENMASK(L0_ORDER + L1_ORDER - 1, 0) & (val))
				...
				struct page *page)
{
    ...
    // Check if the page is zeroed. We do this here because kmap_atomic should
    // be used for short periods of time, and we may end up blocking or
    // allocating memory later.
    src = kmap_atomic(page);
    is_zero = is_zeroed(src);
    kunmap_atomic(src);

    /* init the bitmap if needed */
    spin_lock(&tree->lock);
    ...
    if (!radix_bitmap_is_init(&zswap_zero_bitmap[type])) {
        ...
        spin_lock(&tree->lock);
        // Need to make sure someone else didn't race to init first.
        if (!radix_bitmap_is_init(&zswap_zero_bitmap[type])) {
            radix_bitmap_init(&zswap_zero_bitmap[type], alloc_l0_bitmap);
        } else {
            vfree((void*)alloc_l0_bitmap);
        }
    }
...
success:
    ...
    /* insert the mapping for non-zero pages, and for all pages, remove
     * duplicate insertions. We do this with the lock held. */
    if (is_zero) {
        while((entry = zswap_rb_search(&tree->rbroot, offset))) {
        ...
        if (radix_bitmap_unset(&zswap_zero_bitmap[type],
                RADIX_BITMAP_VAL_MASK(offset)))
            zswap_duplicate_entry++;
				...
				struct page *page)
{
    ...
    /* generate a zero page if is_zero */
    if (is_zero) {
        BUG_ON(entry);
